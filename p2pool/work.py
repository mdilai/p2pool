...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_pubkey_hash, donation_percentage, merged_urls,
                 worker_fee, min_difficulty, share_rate, share_rate_type, args, pubkeys, bitcoind):
    ...
    def get_work(self, user, pubkey_hash, desired_share_target, desired_pseudoshare_target, worker_ip=None):
        ...
        if desired_pseudoshare_target is None:
            ...
            if self.share_rate is not None:
                ...
                if local_rate > 0:
                    target = min(target, bitcoin_data.average_attempts_to_target(local_rate * 600 / self.share_rate))
            else: # per-node
